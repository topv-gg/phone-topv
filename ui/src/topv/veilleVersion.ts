 ...
 * an outdated interface once survived all three and hammered the server at
 * 147 requests a minute for twenty minutes.
 *
 ...
 */
function alreadyReloadedTo(cible: string): boolean {
    try {
...
export function surveillerVersion() {
    const verifier = async () => {
        ...
        if (alreadyReloadedTo(enLigne)) return
        marquerFait(enLigne)
